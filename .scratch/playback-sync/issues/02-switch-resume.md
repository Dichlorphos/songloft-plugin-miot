...
- 2026-09-21（真机验收前第四轮对抗性排查，已采纳）：又发现并修掉 1 个真实缺陷。
  - **采样被判 `stale` 时会写进过期快照**：切换时的位置采样有 2 秒窗口。期间源设备若自动切歌/切歌单，会写入一条更新的出口快照，使本次采样写回被判 `stale`。原先协调器在 `stale` 时只是记日志、继续用**读取时**的旧快照写 pending——目标设备恢复时会播成「已经过去的那首」。现于 `stale` 后重读当前快照，并且**只在该快照仍来自同一台源设备时**采用（快照按账号存一条，同账号别的设备写入的内容不得张冠李戴）。
  - 验证：`npm test` 239 项通过、`npm run typecheck`、`node frontend/tests/run.mjs`、`npm run build`（entryHash `122db1b7…`）。两条关键改动均做过变异验证（去掉重读、去掉源设备校验，对应测试都变红）。

- 2026-09-21（真机验收前第五轮排查）：转向「跨模块契约一致性」检查，并复核规格第 47 行。
  - **契约一致性核对通过**：`playPlaylist` 的 8 个参数在生产接线与全部测试替身中顺序一致；`outcome` 透传点安全（`handled` 为真时必非 `none`，不会把 `none` 泄进响应）。
  - **发现清除时机与规格不符**：规格要求「确认起播成功才清除待播放上下文」，实现是在 `gracefulPlay` 返回 true（仅下发受理）时就清除。设备没拉流时，起播确认会把这台设备判为起播失败并跳歌/停播，而 pending 已被清掉——用户再点「继续」已无可恢复的上下文。已登记为独立票据 [04-landing-confirm-before-clear.md](04-landing-confirm-before-clear.md)（`needs-triage`），不并入本次整改。
  - **本轮另外补强了测试**：`/player/toggle` 的 5 条失败分支与配置校验响应契约（`src/handlers/stop_resume_toggle.test.ts`），覆盖既有实现、非新缺陷，均做变异验证。
