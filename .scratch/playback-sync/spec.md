...
- 用户在目标设备上选择新歌单或新歌曲时立即清除 pending 并使旧同步任务失效，即使尚未点击播放；正式歌单自动切歌成功后立即更新账号级快照，自动切歌失败则保留上一条有效快照。
- 读取 `playback_sync_v1` 时，未知信封 schema 忽略整个信封；单条快照或 pending 字段错误只忽略该条并记录诊断日志，不清空其他账号或设备数据。源设备位置采样超过 2 秒视为失败。
